...
class Client_Socket:
    ...
    async def read_from_server(self, afterHandshake = True):
        server_response = None
        server_response_msg = await self.reader.read(max_msg_size)
        if server_response_msg :
            #decrypt messsage with aesgcm
            if afterHandshake:
                server_response_msg = self.aesgcm.decrypt(server_response_msg[:12],server_response_msg[12:],None)

            server_response = bson.loads(server_response_msg)
        return server_response
    
    """ Envia mensagem recebida em bytes para servidor, encripta com AESGCM"""
    async def send_to_server(self, msg=[], afterHandshake = True):
        if (msg):
            client_msg_bytes = bson.dumps(msg)
            #encrypt message with aesgcm
            if afterHandshake:
                nonce = os.urandom(12)
                ct = self.aesgcm.encrypt(nonce, client_msg_bytes, None)
                client_msg_bytes = nonce + ct

            self.writer.write(client_msg_bytes)
    ...
    async def setup_shared_key(self, keystore_path):
        ...
        await self.send_to_server({"cli_pub_secret":client_public_secret_bytes},False)

        #Read server public secret, signature and certificate
        message = await self.read_from_server(False)

        server_public_secret_bytes, server_signature,cert = message["serv_pub_secret"], message["serv_sign"], message["serv_cert"]
        ...
        message = {"cli_sign":client_signature, "cli_cert":client_certificate.public_bytes(serialization.Encoding.PEM)}

        #Send public secrets signature and certificate to server
        await self.send_to_server(message,False)
        ...
        #save shared key and start aesgcm with that key
        self.key = derived_secret
        self.aesgcm = AESGCM(self.key)

        return client_private_key # returned derived shared key